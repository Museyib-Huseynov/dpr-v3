      ...
      if (file.isDirectory()) {
        ...
        if (extension === '.xls' || extension === '.xlsx') {
          ...
          const platform_id = platforms.find(
            (i) => i.name === Number(platform_number)
          )?.id;
          if (!platform_id) {
          ...
          const flowmeters_entry_exists_query_data = [
            platform_id,
            report_date_id,
          ];
          ...
          const [flowmeters_previous_entry_query_result] =
          ...
          inner: for (let i = 28; i < rows.length; i++) {
            ...
            const error_datalist = must_have_datalist
              .filter(
                (i) =>
                  i[1] === null || i[1] === undefined || i[1]?.trim?.() === ''
              )
              .map((j) => j[0]);
            ...
            const well_stock_entry_exists_query_data = [
              well_id,
              report_date_id,
            ];

            const [well_stock_entry_exists_query_result] =
              await connection.query(
            ...
            const well_stock_previous_entry_query_data = [
              well_id,
              report_date_id,
            ];

            const [well_stock_previous_entry_query_result] =
              await connection.query(
            ...
            const completions_entry_exists_query_data = [
              well_id,
              report_date_id,
            ];

            const [completions_entry_exists_query_result] =
              await connection.query(
            ...
            const completions_previous_entry_query_data = [
              well_id,
              report_date_id,
            ];
            ...
            const [well_tests_entry_exists_query_result] =
              await connection.query(
